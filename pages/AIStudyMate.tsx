...
export const AIStudyMate: React.FC = () => {
  ...
  const startLesson = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      setMessages([{ ...lesson, id: lesson.id || crypto.randomUUID() }]);
    ...
    <div className={`w-full flex flex-col h-full bg-white overflow-hidden animate-in fade-in duration-500 ${lang === 'ar' ? 'text-right' : 'text-left'}`} dir={lang === 'ar' ? 'rtl' : 'ltr'}>
      ...
      <div className="flex-1 flex flex-col overflow-hidden bg-slate-50/30 relative">
        ...
        <div className="flex-1 p-6 overflow-y-auto custom-scroll flex flex-col relative z-10">
          ...
          {messages.length === 0 && !loading && !quizMode && (
            <div className="flex-1 flex flex-col items-center justify-center w-full space-y-12 animate-in zoom-in-95 no-print">
              ...
              <div className="flex flex-wrap justify-center gap-3 max-w-7xl">
                {t.suggestedTopics.map((topicItem, index) => (
                  <button 
                    key={topicItem.ar || `topic-${index}`} 
          ...
          {!quizMode && !certificate && visibleMessages.map((m, mIndex) => (
            <div key={m.id || `msg-${mIndex}`} className="w-full max-w-7xl mx-auto animate-in slide-in-from-bottom-6 duration-700 flex flex-col gap-8 pb-10">
                  ...
                  <div className="space-y-12">
                    <div className="flex flex-wrap justify-center gap-4">
                      {m.infographic.concept_items?.map((v: any, i: number) => (
                        <div key={v.word_ar || `concept-${i}`} className="bg-white p-5 rounded-[2rem] border border-slate-50 shadow-lg flex flex-col items-center text-center w-full sm:w-44 transition-all hover:scale-105 hover:shadow-xl group">
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
                          ...
                          <table className={`w-full ${lang === 'ar' ? 'text-right' : 'text-left'} arabic-font`} dir={lang === 'ar' ? 'rtl' : 'ltr'}>
                            ...
                            <tbody className="divide-y divide-slate-50">
                              {m.infographic.parsing_guide?.map((row: any, i: number) => (
                                <tr key={row.role_ar ? `${row.role_ar}-${i}` : `guide-${i}`} className="hover:bg-blue-50/30 transition-colors">
                      ...
                      <div className="lg:col-span-4 space-y-4">
                        {m.infographic.notes?.map((note: any, i: number) => (
                          <div key={note.ar || `note-${i}`} className="bg-[#fffcf0] border border-amber-100 p-5 rounded-[1.5rem] shadow-sm flex items-start gap-3">
              ...
              {m.options && (
                <div className="flex flex-wrap justify-center gap-3 py-6 no-print">
                  ...
                  {m.options.map((opt, optIdx) => (
                    <button key={`${opt}-${optIdx}`} onClick={() => handleOptionSelect(opt)} className="px-8 py-4 bg-white border border-slate-200 text-[#2563eb] rounded-2xl font-black text-xs hover:border-blue-400 hover:bg-blue-50 transition-all shadow-sm">
                      {opt}
                    </button>
                  ))}
                </div>
              ...
              {!feedback ? (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 shrink-0">
                  {quizQuestions[quizIndex].options?.map((opt: string, optIdx: number) => (
                    <button key={`${opt}-${optIdx}`} onClick={() => handleQuizAnswer(opt)} className="p-4 bg-slate-50 border border-slate-100 hover:border-blue-600 hover:bg-white rounded-xl font-bold text-lg text-slate-700 transition-all arabic-font active:scale-95">{opt}</button>
                  ))}
                </div>
              ) : (
