...
const COUNTRY_DATA = [
  ...
  },
  {
    ...
    nameEn: 'Jordan',
    flag: '🇯🇴',
    image: 'https://images.unsplash.com/photo-1585464233881-23735187445?auto=format&fit=crop&w=1200&q=80',
...
const SafeImage: React.FC<React.ImgHTMLAttributes<HTMLImageElement>> = (props) => {
  ...
  return (
    <img
      ...
      alt={alt}
      {...rest}
      className={`bg-slate-50 transition-all duration-300 ${className || ''}`}
      onError={handleError}
    />
  );
};
...
export const Dialects: React.FC = () => {
  ...
  const [pronunciationResult, setPronunciationResult] = React.useState<any>(null);
  const [mediaRecorder, setMediaRecorder] = React.useState<MediaRecorder | null>(null);
  const audioChunks = React.useRef<Blob[]>([]);

  const audioContextRef = React.useRef<AudioContext | null>(null);
  const recognitionRef = React.useRef<any>(null);
  const recordingTimeoutRef = React.useRef<NodeJS.Timeout | null>(null);
    ...
    try {
      ...
      recorder.onstop = async () => {
        ...
        reader.onloadend = async () => {
          ...
          try {
            const result = await evaluatePronunciation(base64, selectedCountry.vocab[0].dialect, selectedCountry.dialect);
  ...
  return (
    <div className={`w-full flex flex-col h-full bg-white shadow-[0_15px_50px_rgba(0,0,0,0.08)] rounded-[1.5rem] overflow-hidden border border-slate-100 animate-in fade-in duration-700 ${lang === 'ar' ? 'text-right' : 'text-left'}`} dir={lang === 'ar' ? 'rtl' : 'ltr'}>
      ...
      <div className="flex-1 overflow-y-auto custom-scroll p-4 md:p-6 relative bg-slate-50/30">
        ...
        <AnimatePresence mode="wait">
          {!selectedCountry ? (
            ...
            <motion.div 
              ...
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
              className="max-w-[98%] mx-auto bg-white rounded-[2rem] overflow-hidden shadow-2xl border border-slate-100 w-full flex flex-col md:flex-row h-auto md:h-[85vh]"
            >
              {/* Left Column: Info & Culture */}
              <div className="w-full md:w-1/2 flex flex-col overflow-y-auto custom-scroll border-l border-slate-100">
                {/* Hero Section */}
                ...
                <div className="p-5 space-y-6">
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="grid grid-cols-3 gap-2">
                      {selectedCountry.culture.items.map((item: any, i: number) => (
                        <div key={i} className="relative group">
                          ...
                          {/* Tooltip Popup */}
                          <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-40 opacity-0 group-hover:opacity-100 pointer-events-none transition-all duration-300 z-50">
                            <div className="bg-slate-900 text-white p-2 rounded-xl shadow-2xl border border-white/10 text-right">
                              <p className="text-[9px] font-bold arabic-font leading-relaxed mb-1">{item.descAr}</p>
                              <p className="text-[8px] text-slate-400 font-medium leading-tight italic" dir="ltr">{item.descEn}</p>
                    ...
                    <div className="bg-gradient-to-br from-blue-600 to-indigo-700 p-3 rounded-xl border border-white/10 flex items-start gap-2 shadow-lg relative overflow-hidden group">
                      <div className="absolute inset-0 bg-[url('https://www.transparenttextures.com/patterns/carbon-fibre.png')] opacity-10 pointer-events-none" />
                      <div className="w-8 h-8 bg-white/20 backdrop-blur-md rounded-lg flex items-center justify-center text-white shrink-0 shadow-md border border-white/20 group-hover:scale-110 transition-transform">
                        <Sparkles size={16} />
                      </div>
                      <div className="text-right relative z-10">
                        <span className="text-blue-200 font-black uppercase tracking-widest text-[8px] mb-0.5 block">{t.didYouKnow}</span>
                        <p className="text-xs font-bold text-white arabic-font leading-tight">{selectedCountry.culture.factAr}</p>
                        <div className="mt-1 pt-1 border-t border-white/10">
                          <p className="text-[10px] text-blue-100/70 font-medium italic leading-tight" dir="ltr">{selectedCountry.culture.factEn}</p>
                        </div>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="space-y-3">
                    <div className="flex items-center gap-2 border-r-4 border-amber-500 pr-2">
                      <h3 className="text-base font-black text-slate-800 arabic-font">{t.dialogue}</h3>
                    </div>
                    <div className="bg-slate-50 rounded-2xl p-4 border border-slate-100">
                      <h4 className="text-[8px] font-black text-slate-400 uppercase tracking-widest mb-4 text-center">{selectedCountry.dialogue.title}</h4>
                      <div className="space-y-4">
                        {selectedCountry.dialogue.lines.map((line: any, i: number) => (
                          <div key={i} className={`flex flex-col ${i % 2 === 0 ? 'items-start' : 'items-end'}`}>
                            <span className="text-[8px] font-black text-slate-400 uppercase tracking-tighter mb-0.5">{line.role}</span>
                            <div className={`max-w-[85%] p-3 rounded-xl shadow-sm ${i % 2 === 0 ? 'bg-white rounded-tr-none' : 'bg-amber-500 text-white rounded-tl-none'}`}>
                              <p className="text-xs font-black arabic-font mb-0.5">{line.dialect}</p>
                              <p className={`text-[8px] font-bold ${i % 2 === 0 ? 'text-slate-400' : 'text-amber-100'}`} dir="ltr">{line.en}</p>
                            </div>
                          </div>
                        ))}
                      </div>
              ...
              <div className="w-full md:w-1/2 flex flex-col bg-slate-50/50">
                ...
                <div className="flex-1 flex flex-col overflow-hidden">
                  <div className="p-4 border-b border-slate-100 bg-white shrink-0">
                    <div className="flex items-center justify-between mb-3">
                      <h3 className="text-sm font-black text-slate-800 arabic-font">{t.chat}</h3>
                      <div className="flex items-center gap-2">
                        <span className="text-[8px] font-black text-slate-400 uppercase tracking-widest">
                          {lang === 'ar' ? 'اختر شخصيتك' : 'Choose Persona'}
                        </span>
                      </div>
                    </div>
                    <div className="flex gap-2 overflow-x-auto pb-1 custom-scroll">
                      {[selectedCountry.characterMale, selectedCountry.characterFemale].map((char, idx) => (
                        <button
                          key={idx}
                          onClick={() => {
                            setSelectedPersona(char);
                            setChatMessages([]);
                            startLiveConversation(char);
                          }}
                          className={`flex items-center gap-2 p-1.5 rounded-xl border transition-all shrink-0 ${selectedPersona?.nameEn === char.nameEn ? 'bg-blue-600 border-blue-600 text-white shadow-lg' : 'bg-white border-slate-100 text-slate-600 hover:border-blue-200'}`}
                        >
                          ...
                          <div className="text-right pr-1">
                            <div className="text-[10px] font-black arabic-font leading-none mb-0.5">{char.nameAr}</div>
                            <div className={`text-[7px] font-bold uppercase tracking-tighter ${selectedPersona?.nameEn === char.nameEn ? 'text-blue-100' : 'text-slate-400'}`} dir="ltr">{char.nameEn}</div>
                          </div>
                        </button>
                      ))}
                    </div>
                  ...
                  <div className="flex-1 overflow-hidden flex flex-col relative">
                    {/* Pronunciation Challenge Section (New) */}
                    <div className="bg-amber-50/50 border-b border-amber-100 p-4 shrink-0">
                      <div className="flex items-center justify-between mb-3">
                        <div className="flex items-center gap-2">
                          <div className="w-8 h-8 bg-amber-100 rounded-lg flex items-center justify-center text-amber-600">
                            <Mic size={16} />
                          </div>
                          <h4 className="text-xs font-black text-slate-800 arabic-font">{t.pronunciation}</h4>
                        </div>
                        {pronunciationResult && (
                          <div className={`px-2 py-1 rounded-lg text-[10px] font-black ${pronunciationResult.score >= 80 ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-700'}`}>
                            {pronunciationResult.score}%
                          </div>
                        )}
                      </div>
                      
                      <div className="bg-white p-3 rounded-xl border border-amber-100 shadow-sm mb-3">
                        <p className="text-[10px] text-slate-400 font-black uppercase tracking-widest mb-1">{lang === 'ar' ? 'حاول نطق:' : 'Try saying:'}</p>
                        <p className="text-lg font-black text-slate-800 arabic-font text-center py-2">{selectedCountry.vocab[0].dialect}</p>
                        <div className="flex justify-center gap-2 mt-2">
                          <button 
                            onClick={() => playAudio(selectedCountry.vocab[0].dialect)}
                          ...
                          <button 
                            ...
                            className={`flex items-center gap-2 px-4 py-2 rounded-lg font-black text-[10px] uppercase tracking-widest transition-all ${isRecording ? 'bg-red-500 text-white animate-pulse' : 'bg-amber-500 text-white hover:bg-amber-600 shadow-lg shadow-amber-500/20'}`}
                          >
                            {isRecording ? <Square size={14} /> : <Mic size={14} />}
                            {isRecording ? (lang === 'ar' ? 'توقف' : 'Stop') : (lang === 'ar' ? 'ابدأ التسجيل' : 'Start Recording')}
                          </button>
                        </div>
                      </div>
                      ...
                      {isEvaluating && (
                        <div className="flex items-center justify-center gap-2 py-2">
                          ...
                          <span className="text-[10px] font-black text-amber-600 uppercase tracking-widest">{lang === 'ar' ? 'جاري التقييم...' : 'Evaluating...'}</span>
                        </div>
                      )}
                      ...
                      {pronunciationResult && (
                        ...
                        >
                          <p className="text-[11px] font-bold text-slate-700 arabic-font leading-relaxed mb-2">
                            {lang === 'ar' ? pronunciationResult.feedback_ar : pronunciationResult.feedback_en}
                          </p>
                          <div className="flex flex-wrap gap-1">
                            {pronunciationResult.word_accuracy?.map((w: any, idx: number) => (
                              <span 
                                key={idx}
                                className={`px-2 py-0.5 rounded-full text-[9px] font-black ${w.isCorrect ? 'bg-emerald-50 text-emerald-600' : 'bg-rose-50 text-rose-600'}`}
                              >
                                {w.word}
                              </span>
                            ))}
                    ...
                    <div className="flex-1 overflow-y-auto p-4 space-y-3 custom-scroll">
                      {!selectedPersona ? (
                        <div className="h-full flex flex-col items-center justify-center text-center p-6 space-y-4">
                          ...
                          <div>
                            <h4 className="text-xs font-black text-slate-800 arabic-font mb-1">
                              {lang === 'ar' ? 'اختر معلماً للبدء' : 'Choose a tutor to start'}
                            </h4>
                            <p className="text-[9px] text-slate-400 font-bold uppercase tracking-widest leading-relaxed">
                              {lang === 'ar' ? 'تحدث بلهجتنا المحلية وتعلم بذكاء' : 'Speak our local dialect and learn smartly'}
                            </p>
                          </div>
                        </div>
                        ...
                        <div className="h-full flex flex-col items-center justify-center text-center p-6 space-y-4">
                          ...
                          <div>
                            <h4 className="text-xs font-black text-slate-800 arabic-font mb-1">
                              {lang === 'ar' ? `ابدأ المحادثة مع ${selectedPersona.nameAr}` : `Start chatting with ${selectedPersona.nameEn}`}
                            </h4>
                          </div>
                        </div>
                      ) : (
                        chatMessages.map((msg, i) => (
                          ...
                          >
                            <div className={`max-w-[85%] p-3 rounded-2xl text-xs font-bold arabic-font shadow-sm ${msg.role === 'user' ? 'bg-blue-600 text-white rounded-tr-none' : 'bg-white text-slate-800 border border-slate-100 rounded-tl-none'}`} dir="auto">
                              {msg.text}
                            </div>
                          </motion.div>
                        ))
                ...
                <div className="p-4 bg-white border-t border-slate-100 flex items-center justify-between shrink-0">
                  <div className="flex items-center gap-2">
                    ...
                    >
                      <Zap size={14} />
                      {lang === 'ar' ? 'محادثة حية' : 'Live Chat'}
                    </button>
                    
                    <button 
                      onClick={() => {
                        // Scroll to pronunciation section if needed or just highlight it
                        const el = document.querySelector('.bg-amber-50\\/50');
                        el?.scrollIntoView({ behavior: 'smooth' });
                      }}
                      className="flex items-center gap-2 px-4 py-2 bg-amber-500 text-white rounded-xl font-black text-[10px] uppercase tracking-widest hover:bg-amber-600 transition-all shadow-lg shadow-amber-500/20"
                    >
                      <Mic size={14} />
                      {t.pronunciation}
                    </button>
